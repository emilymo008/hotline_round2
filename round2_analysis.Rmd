...
surv$scout_name[surv$scout_name == 'José A'] <- 'José A'  # some sort of encoding issue on the accent 
surv <- surv %>% left_join(transcripts)
which(is.na(surv$`Q33: You're almost done! Which of the following would you prefer when it comes to the overall patient support experience?`))  # should be 0

# write.csv(surv, 'clean_survey_INCOMPLETE.csv')
```
...
Order factors
```{r}
surv$missing_info_oriented <- factor(surv$missing_info_oriented, levels = c(
  "I would feel not at all oriented to why I received this message.", 
...
surv$welcome_screen_mi <- factor(surv$welcome_screen_mi, levels = c(
...
surv$bibv_faq <- factor(surv$bibv_faq, levels = c(
  "Very difficult",
  "Difficult",
...
surv$bibv_action <- factor(surv$bibv_action, levels = c(
  "Very unclear",
  "Unclear",
...
surv$thirdparty_headsup <- factor(surv$thirdparty_headsup, levels = c(
  "Completely unnecessary to have",
  "Slightly helpful to have, but not necessary",
  "Moderately helpful to have, but not necessary",
  "Very helpful to have, but not necessary",
  "Completely necessary to have"
))

surv$thirdparty_availability <- factor(surv$thirdparty_availability, levels = c(
  "Completely unnecessary to have",
  "Slightly helpful to have, but not necessary",
  "Moderately helpful to have, but not necessary",
  "Very helpful to have, but not necessary",
...
surv$bridge_info <- factor(surv$bridge_info, levels = c(
  "I would have needed a lot more information on the page to help me understand how the Bridge Program works.",
  "I would have needed a bit more information on the page to help me understand how the Bridge Program works.",
  "There was just the right amount of information on the page to help me understand how the Bridge Program works.",
  "I could have used a bit less information on the page to understand how the Bridge Program works.",
  "I could have used a lot less information on the page to understand how the Bridge Program works."
))

surv$bridge_ending <- factor(surv$bridge_ending, levels = c(
  "Very unclear",
  "Unclear",
  "Neither clear nor unclear",
  "Clear",
...
surv$sp_action <- factor(surv$sp_action, levels = c(
  "Very unclear",
  "Unclear",
  "Neither clear nor unclear",
  "Clear",
...
surv$copay_dashboard <- factor(surv$copay_dashboard, levels = c(
  "Not at all helpful",
  "Slightly helpful",
  "Somewhat helpful",
  "Helpful",
...
surv$copay_balance <- factor(surv$copay_balance, levels = c(
...
surv$reduce_calls <- factor(surv$reduce_calls, levels = c(
  "Does not at all reduce my likelihood to proactively contact my drug manufacturer’s patient services center via phone.",
  "Slightly reduces my likelihood to proactively contact my drug manufacturer’s patient services center via phone.",
  "Moderately reduces my likelihood to proactively contact my drug manufacturer’s patient services center via phone.",
  "Mostly reduces my likelihood to proactively contact my drug manufacturer’s patient services center via phone.",
  "Completely reduces my likelihood to proactively contact my drug manufacturer’s patient services center via phone."
))
...
# Analysis
```{r}
# orientation to MI sliced by experience with SP

# option a vs b timeline sliced by experience with SP
tabyl(surv, historical_sp, timeline_prefer) %>% adorn_percentages() %>% adorn_ns()  # people with historical experience with sp might be more open to B - do a test

# option a vs b timeline sliced by experience with PA
tabyl(surv, historical_pa, timeline_prefer) %>% adorn_percentages() %>% adorn_ns()  # same
